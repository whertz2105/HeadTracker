...
HardwareSerial& radioSerial = Serial2;
...
void loop() {
  ...
  // Receive packets from HC-12.
  while (radioSerial.available() >= static_cast<int>(kPacketSize)) {
    HeadsetPacket pkt;
    radioSerial.readBytes(reinterpret_cast<char*>(&pkt), kPacketSize);
    if (!validatePacket(pkt)) {
      Serial.println(F("[Drone] CRC mismatch."));
      continue;
    }
    packet_valid = true;
